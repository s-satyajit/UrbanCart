...
const geminiEndpoint = (model) => `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${env.geminiApiKey}`;
...
async function callGemini(payload) {
    ...
    if (!response.ok) {
        throw badRequestError(data?.error?.message || "Gemini could not answer the question right now.");
    }
    return data;
}
                ...
                parts: [
                    {
                        text: [
                            ...
                            ...channels.map((channel) => `- ${channel.title}: ${channel.value} (${channel.description})`),
                        ].join("\n"),
                    },
                ],
            },
        ...
        generationConfig: {
            ...
            responseJsonSchema: {
                ...
                properties: {
                    ...
                    howCanWeHelp: {
                        type: "string",
                        description: "A concise but useful website contact form message explaining the support request.",
                    },
                },
...
export async function interpretProductSearch({ query, categories = [], brands = [], badges = [], titles = [], }) {
    const fallback = {
